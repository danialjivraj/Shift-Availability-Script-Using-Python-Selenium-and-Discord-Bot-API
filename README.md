...
Although there was official shift releases, of which they would email me, every Monday, it would usually not be the days or shifts I wanted.<br>
Now for example, the shift "10:00 - 18:00" on the Saturdays could be available for only 50 people, and the first 50 people to book them get it. Now suppose, let's say Bob, was the 50th person to book this shift, the day would turn Grey (if there was also no other shifts available). But Bob could for example take his shift off 1-3 days before his Saturday shift, so for example if he took his shift off on Friday, that would mean that the 10:00 - 18:00 would be available again, but the problem is that no one gets notified about this available shift, so unless you were in the website at that exact moment when Bob unbooked himself from the shift, there would be no way of knowing about the shift.
...
## How It Works
1. **User Commands:** When the trigger command is typed in the Discord channel it executes the corresponding actions.
...
5. **Filtering:** Filters shifts based on user preferences, such as specific days and shifts.
6. **Notification:** Notifies users about available shifts through Discord messages when it meets the conditions.
...
## .env
I stored my Discord ID, Channel ID and the Discord Token inside of here to keep it private. <br>
I hid my login information for obvious reasons and the URL's for the website as well.<br>
...
I didn't know what Selenium and Chrome Driver was before working on this project. Nor did I ever use a Discord Bot API in order to send messages, so combining both of these technologies together allowed me to create something extremely affective, and allowed me to improve my Python skills overall.<br>
